...
if __name__ == "__main__":
    ...
    # logging.info(f'[n,mx,mz] = [{new_hx.shape[1]},{len(new_hx)},{len(new_hz)}]')
    
    # hz = add_symm_from_hz(new_hx, new_hz)
    # logging.info(f'n_row of new Hz: {len(hz)}; rank of new Hz: {rank(hz)}')

    hz = np.load('hz_gauging_0.npy').astype(np.int64)

    ns_x = nullspace(new_hx).astype(np.int64)
    ns_x_gauge = ns_x[np.sum(ns_x, axis=1) > 1].astype(np.int64)
    logging.info(f'n_row of ns_x_gauge: {len(ns_x_gauge)}')
    ns_x = ns_x[np.sum(ns_x, axis=1) > 1].astype(np.int64)
    ns_x = ns_x[np.argsort(np.sum(ns_x, axis=1))].astype(np.int64)
    # hz = gauging(new_hx, hz, ns_x, ns_x_gauge, transform_idx=0)
    hz = gauging(new_hx, hz, ns_x, ns_x_gauge, transform_idx=1)
    logging.info(f'n_row of Hz after gauging: {len(hz)}; rank of Hz after gauging: {rank(hz)}')
    
    np.save(os.path.join(f'{pid}_hz_gauging.npy'), hz)

    logging.info(f'rank of Hx: {rank(new_hx)}')
    logging.info(f'rank of Hz: {rank(hz)}')

    assert check_comm_mat_mat(new_hx, hz)
    
    lz = compute_lz(new_hx, hz)
    lx = compute_lz(hz, new_hx)

    logging.info(f'k = {len(lx)}')

    lx = lx[np.argsort(np.sum(lx, axis=1))]
    logging.info(f'min wieght lx: {np.sum(lx[0])}')

    plt.hist(np.sum(lx, axis=1), bins=range(0, 10))
    plt.xlabel('Weight')
    plt.ylabel('Frequency')
    plt.title('Weight distribution of Lx')
...
#     # plt.xlabel('Weight')
#     # plt.ylabel('Frequency')
#     # plt.title('Weight distribution of classical Hx codewords')
#     # plt.savefig('../../figures/6to3/classical_Hx_codewords_weight_dist.png')
#     # plt.show()

#     c2v_wt = np.sum(new_hx, axis=1)
#     assert len(c2v_wt) == len(new_hx)
#     v2c_wt = np.sum(new_hx, axis=0)
#     assert len(v2c_wt) == len(new_hx[0])
#     plt.hist(c2v_wt, bins=range(0, 10), alpha=0.5, label='C2V')
#     plt.hist(v2c_wt, bins=range(0, 10), alpha=0.5, label='V2C')
#     plt.annotate(f'unchecked qubits: {np.sum(v2c_wt == 0)}', xy=(0, 0), xytext=(0, 0), textcoords='axes fraction')
#     plt.xlabel('Weight')
#     plt.ylabel('Frequency')
#     plt.legend()
#     plt.title('Weight distribution of Hx')
#     plt.savefig('../../figures/6to3/Hx_weight_dist.png')
#     plt.show()

#     # ns_x_gauge = ns_x[np.sum(nullspace(new_hx), axis=1) == 1]
#     # bin_vec = _transform_idx_to_bin(4, len(ns_x_gauge))
#     # transform_element = (bin_vec @ ns_x_gauge) % 2
#     # logging.info(f'transform_element: {transform_element}')
#     # ns_x_transform = (ns_x + transform_element) % 2
#     # logging.info(f'ns_x_transform[0]: {ns_x_transform[0]}')

#     # diff_mat = (ns_x_transform + ns_x)%2
#     # for i in range(len(ns_x)):
#     #     assert np.allclose(diff_mat[i], transform_element[0])

#     # hz = add_symm_from_hz(new_hx, new_hz)
#     # logging.info(f'ker new_Hx: {len(ns_x)}; img Hz.T: {len(row_basis(hz.T))}')
#     # lz = compute_lz(new_hx, hz)
#     # lx = compute_lz(hz, new_hx)
#     # # logging.info(f'k = {len(lx)}')
#     # logging.info(f'min wieght lx: {np.sum(lx[0])}')
#     # logging.info(f'min wieght lz: {np.sum(lz[0])}')

#     # plt.hist(np.sum(lx, axis=1), bins=range(0, 10), alpha=0.5, label='Lx')
#     # plt.hist(np.sum(lz, axis=1), bins=range(0, 10), alpha=0.5, label='Lz')
#     # plt.xlabel('Weight')
#     # plt.ylabel('Frequency')
#     # plt.legend()
#     # plt.title('Weight distribution of Lz')
#     # plt.savefig('Hxorigin_Hzsymm_lx_lz_weight_dist.png')
#     # plt.show()
