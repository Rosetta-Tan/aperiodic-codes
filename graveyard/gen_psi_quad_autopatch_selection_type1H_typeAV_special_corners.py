...
'''Gen 20'''
readdir = '/Users/yitan/Library/CloudStorage/GoogleDrive-yitan@g.harvard.edu/My Drive/from_cannon/qmemory_simulation/data/qc_code/psi_tiling/'
savedir = '/Users/yitan/Library/CloudStorage/GoogleDrive-yitan@g.harvard.edu/My Drive/from_cannon/qmemory_simulation/data/qc_code/psi_tiling/gen_20/autopatch_1HAV'
# readdir = '..\data\qc_code\psi_tiling'
...
'''Gen 21'''
# readdir = '/Users/yitan/Library/CloudStorage/GoogleDrive-yitan@g.harvard.edu/My Drive/from_cannon/qmemory_simulation/data/qc_code/psi_tiling/'
# savedir = '/Users/yitan/Library/CloudStorage/GoogleDrive-yitan@g.harvard.edu/My Drive/from_cannon/qmemory_simulation/data/qc_code/psi_tiling/gen_21/autopatch_1HAV'
# data = np.load(os.path.join(readdir, 'psi_tiling_gen_21.npz'))
'''Gen 25'''
# readdir = '/Users/yitan/Library/CloudStorage/GoogleDrive-yitan@g.harvard.edu/My Drive/from_cannon/qmemory_simulation/data/qc_code/psi_tiling/'
# savedir = '/Users/yitan/Library/CloudStorage/GoogleDrive-yitan@g.harvard.edu/My Drive/from_cannon/qmemory_simulation/data/qc_code/psi_tiling/gen_25/autopatch_1HAV'
...
def get_local_env_horizontal(v):
    ...
    if face_ctg_counter[0] == 3 and face_ctg_counter[1] == 0 and face_ctg_counter[2] == 1:
        ...
        return 5
    # Special treatments
    elif face_ctg_counter[0] == 1 and face_ctg_counter[1] == 0 and face_ctg_counter[2] == 1:
        return 1
    elif face_ctg_counter[0] == 1 and face_ctg_counter[1] == 1 and face_ctg_counter[2] == 0:
        return 2
    elif face_ctg_counter[0] == 1 and face_ctg_counter[1] == 1 and face_ctg_counter[2] == 1:
        return 2
...
def get_local_env_vertical(v):
    ...
    # special treatments
        ...
        return 2
    elif face_ctg_counter[0] == 2 and face_ctg_counter[1] == 1 and face_ctg_counter[2] == 0:
        return 2
    else:
        return 0
...
def get_boundary(v_start, step_h, step_v):
    '''Specify the four corners'''
    v_top_right = np.asarray(v_start)
    assert get_local_env_horizontal(v_top_right) in [1,3]
    face_ind_top_right =face_matching_in_local_env(v_top_right, ctg=2, order=2)
    ...
    while down_travel_counter <= step_v:
        v = move_down(v)
        boundary.append(v)
        # if get_local_env_vertical(v) in [1,5]:
        down_travel_counter += 1
        if down_travel_counter == step_v:
            break

    # move left
    left_travel_counter = 0
    while left_travel_counter <= step_h:
        v = move_left(v)
        boundary.append(v)
        # if get_local_env_horizontal(v) in [2,4,5]:
        left_travel_counter += 1
        if left_travel_counter == step_h:
            break

    v_ind_bot_left = len(boundary)-1
    ...
    while right_travel_counter <= step_h:
        v = move_right(v)
        boundary.append(v)
        # if get_local_env_horizontal(v) in [1,2,3,4]:
        right_travel_counter += 1
        if right_travel_counter == step_h:
            break
    
    assert close(v, v_top_right)
    boundary = boundary[:-1]

    # # corner rounding (general)
    # if get_local_env_horizontal(boundary[v_ind_bot_left]) in [2,3,4]:
    #     boundary.pop(v_ind_bot_left)

    '''special treatment for single-diamond (2,1) truncation'''
    # boundary_front = boundary[:v_ind_bot_left-1]
    # boundary_end = boundary[v_ind_bot_left+4:]
    # boundary_middle = []
    # face_ind = face_matching_in_local_env(boundary[v_ind_bot_left-2], ctg=0, order=4)
    # v_temp = FACES[face_ind][2]
    # boundary_middle.append(v_temp)
    # face_ind = face_matching_in_local_env(v_temp, ctg=2, order=3)
    # v_temp = FACES[face_ind][1]
    # boundary_middle.append(v_temp)
    # boundary = boundary_front + boundary_middle + boundary_end

    '''special treatment for single-diamond (1,2) truncation'''
    boundary_front = boundary[:v_ind_bot_left-1]
    boundary_end = boundary[v_ind_bot_left+3:]
    boundary_middle = []
    face_ind = face_matching_in_local_env(boundary[v_ind_bot_left-2], ctg=0, order=4)
    v_temp = FACES[face_ind][2]
    boundary_middle.append(v_temp)
...
assert get_local_env_horizontal(v_start) in [1,3]
len_h = 19
len_v = 29
savedir = os.path.join(savedir, f'start_index={start_ind}_len_h={len_h}_len_v={len_v}')
if not os.path.exists(savedir):
    os.makedirs(savedir)
...
for i in range(len(boundary_vertices)-1):
    ax.plot([boundary_vertices[i][0], boundary_vertices[i+1][0]], [boundary_vertices[i][1], boundary_vertices[i+1][1]], color='red', zorder=10)
    ax.plot([boundary_vertices[-1][0], boundary_vertices[0][0]], [boundary_vertices[-1][1], boundary_vertices[0][1]], color='red', zorder=10)
ax.set_aspect('equal')
ax.set_axis_off()
fig.tight_layout()
...
# xs_after_removal = [x for i, x in enumerate(xs) if i not in columns_to_remove]
# ys_after_removal = [y for i, y in enumerate(ys) if i not in columns_to_remove]
ax.scatter(xs_after_removal, ys_after_removal, s=300, marker='o')
# annotate the points
# for ipt in range(len(xs)):
#     ax.annotate(ipt, (xs[ipt], ys[ipt]), zorder=2)
ax.scatter(xs[pos_ones], ys[pos_ones], marker='o', s=400, color='pink', zorder=1)
# ax.scatter(xs[pos_ones], ys[pos_ones], marker='o', color='pink', zorder=1)
