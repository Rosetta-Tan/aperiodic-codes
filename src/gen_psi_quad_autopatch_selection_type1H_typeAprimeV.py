...
'''Gen 23'''
# readdir = '/Users/yitan/Library/CloudStorage/GoogleDrive-yitan@g.harvard.edu/My Drive/from_cannon/qmemory_simulation/data/qc_code/psi_tiling/'
# savedir = '/Users/yitan/Library/CloudStorage/GoogleDrive-yitan@g.harvard.edu/My Drive/from_cannon/qmemory_simulation/data/qc_code/psi_tiling/gen_23/autopatch'
# data = np.load(os.path.join(readdir, 'psi_tiling_gen_23.npz'))
# '''Gen 24'''
# readdir = '/Users/yitan/Library/CloudStorage/GoogleDrive-yitan@g.harvard.edu/My Drive/from_cannon/qmemory_simulation/data/qc_code/psi_tiling/'
# savedir = '/Users/yitan/Library/CloudStorage/GoogleDrive-yitan@g.harvard.edu/My Drive/from_cannon/qmemory_simulation/data/qc_code/psi_tiling/gen_22/autopatch'
# data = np.load(os.path.join(readdir, 'psi_tiling_gen_22.npz'))
...
def get_boundary(v_start, step_h, step_v):
    ...
    for _ in range(step_h):
        v = move_right(v)
        boundary.append(v)
    while get_local_env_horizontal(v) not in [1,2,3,4]:
        v = move_right(v)
        boundary.append(v)
    
    # record top right vertex and its face, make transition
    v_top_right = v
    v_ind_top_right = len(boundary)-1
    face_ind_top_right = face_matching_in_local_env(v, ctg=0, order=1)  # ctg=0, order=A
    v = FACES[face_ind_top_right][2]  # move from ctg=0 face A to B
    boundary.append(v)
    ...
    for _ in range(step_v):
        v = move_down(v)
        boundary.append(v)
    while get_local_env_vertical(v) not in [1,5]:
        v = move_down(v)
        boundary.append(v)
    
    # record bottom right vertex and its face
    v_bottom_right = v
    face_ind_bottom_right = face_matching_in_local_env(v, ctg=0, order=4) # ctg=0, order=D
    v = FACES[face_ind_bottom_right][1]  # move from ctg=0 face D to A
    boundary.append(v)
    ...
    for _ in range(step_h):
        v = move_left(v)
        boundary.append(v)
    while get_local_env_horizontal(v) not in [2,4,5]:
        v = move_left(v)
        boundary.append(v)

    # record bottom left vertex and its face
    v_bottom_left = v
    v_ind_bottom_left = len(boundary)-1
    face_ind_bottom_left = face_matching_in_local_env(v, ctg=0, order=3) # ctg=0, order=C
    v = FACES[face_ind_bottom_left][4]  # move from ctg=0 face C to D
    boundary.append(v)
    ...
    for _ in range(step_v):
        v = move_up(v)
        boundary.append(v)
        if get_local_env_vertical(v) in [1,2,3,4] and face_matching_in_local_env(v, ctg=0, order=2) == face_ind_top_left:
            terminate = True
            break
    while not terminate:
        v = move_up(v)
    ...
    # corner rounding
    boundary.pop(v_ind_bottom_left+1)
    if get_local_env_horizontal(v_bottom_left) == 5:
        boundary.pop(v_ind_bottom_left)
    face_ind_top_right_special = face_matching_in_local_env(v_top_right, ctg=0, order=2)
    boundary[v_ind_top_right+1] = FACES[face_ind_top_right_special][4]
...
start_ind = 1932 # starting point can only be of local env 2,4,5
v_start = vertices_pos[start_ind]
assert get_local_env_horizontal(v_start) in [2,4,5]
len_h = 22
len_v = 19
...
# for i in range(60,64):
    ...
    ax.scatter(xs[pos_ones], ys[pos_ones], marker='o', s=10, color='pink', zorder=1)
